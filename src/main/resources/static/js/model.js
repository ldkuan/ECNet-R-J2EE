...
function modelTransfer(modelJson) {
	...
	for (var i = 0; i < evidences.length; i++) {
		...
		var evidence_show = drawBody(true, pre_bx + gap_x * 0.5, pre_by
				+ (gap_y * head_count) / 2, body['id'], body['name'],
				body['body'], body['type'], body['submitter'], body['reason'],
				body['result'], body['documentid'], body['isDefendant'],
				body['trust']);
		...
		if (tempHeadList.length > 0) {
			...
			for (var j = 0; j < tempHeadList.length; j++) {
				...
				var header_show = drawHeader(true, pre_hx + gap_x * 2.5, pre_hy
						+ gap_y * hy_times, header_id, header, header);
	...
	for (var i = 0; i < allfacts.length; i++) {
		...
		// draw facts
		var fact_show = drawFact(true, pre_fx + gap_x * 6, pre_fy
				+ (gap_y * joint_count) / 2, tempFact["id"], tempFact["name"],
				tempFact["content"], tempFact["type"]);
		...
		if (tempJoints.length > 0) {
			...
			for (var j = 0; j < tempJoints.length; j++) {
				...
				if (joint_name_list.indexOf(entry['value']) == -1) {
					...
					var joint_show;
					joint_show = drawJoint(true, pre_jx + gap_x * 4.5, pre_jy
							+ gap_y * jy_times, "", entry['value'],
							entry['value'], "");
					jy_times++;
