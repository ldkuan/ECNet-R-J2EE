...
function initEvidences() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			initGraph(data['trusts'], data['freeHeaders'], data['joints'],
					data['arrows'], data['facts']);
...
function initRejection(evidences) {
	...
	for (var i = 0; i < evidences.length; i++) {
		...
		for (var j = 0; j < evidenceTemp['headers'].length; j++) {
			content += "<span class=\"head_chain\">"
					+ evidenceTemp['headers'][j] + "</span>";
		}
		content += "</div></div></div>";
	}
...
// }
// content+="<span class=\"head_chain\"
// "+dataattr_header+">"+evidenceTemp['链头'][j]+"</span>";
// }
// content+="</div></div></div>";
// }
...
function addEvidence(id, evidence_content, isDefendant) {
	...
	if (p_div == null || p_div.length == 0) {
		var div_html = "<div class=\""
				...
				+ id
				+ "'>\n"
				+ "                            <a data-toggle=\"collapse\" href=\"#heads_chain"
				+ id + "\" class=\"evidence_a\">\n" + evidence_content
				+ "</a></div>";
...
function addHeaderofChain(header_content, header_id, body_id) {
	...
	if (p_div != null && p_div.length > 0) {
		...
		if (p_div.find('.head_div') == null
				|| p_div.find('.head_div').length == 0) {

			var add_html = " <div id=\""
					+ id
					+ "\" class=\"panel-collapse collapse in\">\n"
					+ "                                <div class=\"head_div\">"
					+ "<span class=\"head_chain\" data-id='" + header_id + "'>"
					+ header_content + "</span></div></div>";
			p_div.append(add_html);
		} else {
			var header_div = p_div.find('span[data-id=' + header_id + ']');

			if (header_div == null || header_div.length == 0) {
				var add_html = "<span class=\"head_chain\" data-id='"
						+ header_id + "'>" + header_content + "</span>";
